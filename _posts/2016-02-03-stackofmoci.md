...
1년이 넘는 시간 동안 250만명 이상의 회원이 일으키는 트래픽을
수 십대의 Ubuntu 14.04 로 운영해 왔습니다만, Ubuntu 서버는
별 다른 문제를 일으킨 적이 없습니다.
...
순 조회수를 정확하게 기록함과 동시에 메인 데이터베이스에
부하를 분산하기 위한 의도입니다. MongoDB 는 3.0 때 도입된
WiredTiger 스토리지 엔진으로 데이터 저장 효율이 비약적으로
향상 되었습니다.
...
나오지는 않습니다. 또한 ORM 레이어를 기반으로 MongoDB 를
사용할 경우 데이터를 막(?) 때려넣고 막(?) 조회할 수 있는
장점을 포기한다고 볼 수도 있을 것 같습니다. 아무쪼록
주의깊게 사용할 필요가 있습니다.
...
S3 에 업로드 된 수 테라 바이트의 이미지는 AWS CloudFront CDN 으로
...
이용하여 로드밸런싱을 하도록 설정 하였습니다. HaProxy 의 경우 사실상
무제한의 idle timeout 을 설정할 수 있기 때문에 이 경우 유용한
대안이 될 수 있습니다. 
...
중 2명만이 서버 개발을 진행했기 때문에 힘에 부치는 일도 많았고, 
